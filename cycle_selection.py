...
def get_cnr_all_frames(path_to_images, path_to_segmentations, images_of_one_view, frames_to_exclude):
    ...
    for frame_nr, name in enumerate(images_of_one_view):
        if frame_nr not in frames_to_exclude:
            ...
            file_location_image = os.path.join(path_to_images, (name[:-7] + '_0000' + name[-7:]))
...
def find_es_in_cycle(ES_points, ED_points_cycle, LV_areas):
    ...
    ES_points_cycle = [ES_point for ES_point in ES_points if ES_point > ED_points_cycle[0] and ES_point < ED_points_cycle[1]]
...
def score_criteria(my_list, method):
    ...
    if method == 'max all':
        scores = [4 if item == max_list else 0 if item == min_list else 2 for item in my_list_rounded]
    elif method == 'min all':
        scores = [4 if item == min_list else 0 if item == max_list else 2 for item in my_list_rounded]
...
def get_best_cycle(cnr_cycles, nr_of_outliers_sf_qc, nr_of_outliers_mf_qc):
    ...
    if scores_tot.count(max_score) > 1:
        idx_list = [idx for idx, score_val in enumerate(scores_tot) if score_val == max_score]
...
def get_main_cycle(cnr_frames, ed_points, es_points, lv_areas, frames_to_exclude_sf_qc, frames_to_exclude_lv_mf_qc, frames_to_exclude_la_mf_qc):
    """ Function to select the most appropriate cardiac cycle.
    ...
    """
    cnr_cycles, ed_points_cycles, es_points_cycles, nr_outliers_sf_qc_cycles, nr_outliers_mf_qc_cycles = [], [], [], [], []
    ...
    for idx in range(len(ed_points) - 1):
        ...
        count_outliers_sf_qc = nr_outliers_in_cycle(frames_to_exclude_sf_qc, ed_points[idx], ed_points[idx+1])     
        nr_outliers_sf_qc_cycles.append(count_outliers_sf_qc)
        
        # Count number of outliers multi-frame QC. 
        count_outliers_lv_mf_qc = nr_outliers_in_cycle(frames_to_exclude_lv_mf_qc, ed_points[idx], ed_points[idx+1])
        count_outliers_la_mf_qc = nr_outliers_in_cycle(frames_to_exclude_la_mf_qc, ed_points[idx], ed_points[idx+1])
        nr_outliers_mf_qc_cycles.append((count_outliers_lv_mf_qc + count_outliers_la_mf_qc))
    
    # Get CNR of best cycle and corresponding ED and ES points.
    cnr_best_cycle, score = get_best_cycle(cnr_cycles, nr_outliers_sf_qc_cycles, nr_outliers_mf_qc_cycles)
