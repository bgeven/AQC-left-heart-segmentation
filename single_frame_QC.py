...
import cv2
import numpy as np
from general_utilities import find_contours, combine_segmentations, get_image_array, separate_segmentation
...
def check_for_gaps(num_of_contours_ext, contours_all, min_size=[2,2]):
    ...
    filtered_contours = [contour for contour in contours_all if cv2.contourArea(contour) >= (min_size[0] * min_size[1])]
...
def check_gap_between_structures(segA, segB, num_gapsA, num_gapsB, min_size=[1,1]):  
    ...
    filtered_contours = [contour for contour in contoursAB if cv2.contourArea(contour) >= (min_size[0] * min_size[1])]
...
def check_for_gaps_full(contours, num_gaps1, num_gaps2, num_gaps3, num_gaps12, num_gaps13, min_size=[1,1]):
    """ Check if gaps are present in a segmentation.
    ...
    """
    # Pick contours larger than the defined minimum value. 
    filtered_contours = [contour for contour in contours if cv2.contourArea(contour) >= (min_size[0] * min_size[1])]
    ...
    num_other_gaps = (len(filtered_contours) - num_gaps1 - num_gaps2 - num_gaps3 - num_gaps12 - num_gaps13 - 1)
...
def do_single_frame_QC(directory_segmentations, images_of_one_person):   
    ...
    for image in images_of_one_person:
        ...
        num_gaps12 = check_gap_between_structures(seg1, seg2, num_gaps1, num_gaps2, min_gap_size)
        num_gaps13 = check_gap_between_structures(seg1, seg3, num_gaps1, num_gaps3, min_gap_size)
        num_gaps23 = check_for_gaps_full(contours0_all, num_gaps1, num_gaps2, num_gaps3, num_gaps12, num_gaps13, min_gap_size)
...
def main_single_frame_QC(path_to_segmentations):
    ...
    for patient in patients:
        ...
        QC_scores, overview, flagged_frames = do_single_frame_QC(path_to_segmentations, images_of_one_person)
