...
from general_utilities import find_contours, find_largest_contour, get_image_array, separate_segmentation, combine_segmentations, find_coordinates_of_holes
...
def get_main_contour_LV_or_LA(seg, mean_centroid):   
    ...
    selected_contour = [contour for contour in contours if (
        cv2.pointPolygonTest(contour, mean_centroid, False) >= 0 or 
        cv2.pointPolygonTest(contour, (mean_centroid[0]-25, mean_centroid[1]), False) >= 0 or 
        cv2.pointPolygonTest(contour, (mean_centroid[0]+25, mean_centroid[1]), False) >= 0 or 
        cv2.pointPolygonTest(contour, (mean_centroid[0], mean_centroid[1]-25), False) >= 0 or 
        cv2.pointPolygonTest(contour, (mean_centroid[0], mean_centroid[1]+25), False) >= 0)]
...
def get_main_contour_MYO(seg_1, seg_2): 
    ...
    cv2.drawContours(seg_main, [contours_2[k] for k in neighboring_contours], -1, 1, thickness=-1)
    ...
    if len(neighboring_contours) > 1:
        seg_main = cv2.morphologyEx(seg_main, cv2.MORPH_CLOSE, np.ones((25,25),np.uint8))    
...
def find_coordinates_of_structure(seg, label):
    ...
    Returns:
        coordinates (list): List of tuples of x- and y-coordinates of pixels in structure.
    """	
...
def fill_holes_between_LV_LA(segmentation):
    ...
    for row, col in zip(coordinates_holes_13[0], coordinates_holes_13[1]):
        if (col, row) not in coordinates_seg_1 and (row, col) not in list(zip(coordinates_holes_12[0], coordinates_holes_12[1])):
...
def find_border_pixels_holes(distances):
    ...
    Returns:
        indices (list): List of tuples of start and end indices of the holes between the structures.
    """
    indices = []
    start_idx = None
...
def fill_holes_between_LV_MYO(seg_total):
    ...
    if len(contour_2) > 0 and len(contour_3) > 0: 
        min_distances_12 = [abs(cv2.pointPolygonTest(contour_2[0], tuple(map(int, point[0])), True)) for point in contour_1[0]]
        min_distances_13 = [abs(cv2.pointPolygonTest(contour_3[0], tuple(map(int, point[0])), True)) for point in contour_1[0]]
        ...
        if min(min_distances_12) <= 1.0 and min(min_distances_13) <= 1.0:
            ...
            for hole in holes:
                ...
                coor_x1, coor_y1 = contour_1[0][hole[0]][0][0], contour_1[0][hole[0]][0][1] 
                coor_x2, coor_y2 = contour_1[0][hole[1]][0][0], contour_1[0][hole[1]][0][1]
                
                # Create list with indices where contour 2 neighbors contour 1. 
                list_j = [j for j, coor in enumerate(contour_2[0]) if abs(cv2.pointPolygonTest(contour_1[0], tuple([int(coor[0][0]), int(coor[0][1])]), True)) <= 1]
                
                # Final all points on contour of myocardium and remove all inside points. 
                contour_points = np.array(contour_2[0])[:,0].reshape(-1,2)
                outside_c_points_x = list(contour_points[:,0][:list_j[0]]) + list(contour_points[:,0][list_j[-1]:])
                outside_c_points_y = list(contour_points[:,1][:list_j[0]]) + list(contour_points[:,1][list_j[-1]:])
              
                # Calculate the minimum difference between the gap points to the outside of the myocardium. 
                distances_gap1 = [np.sqrt((x - coor_x1) ** 2 + (y - coor_y1) ** 2) for x, y in zip(outside_c_points_x, outside_c_points_y)]
...
def post_process_segmentation(segmentation, centroids):
    ...
    """	
    # Separate the segmentations, each with its own structures.
    _, seg_1_before_processing, seg_2_before_processing, seg_3_before_processing = separate_segmentation(segmentation)
    ...
    seg_1, num_of_contours1 = get_main_contour_LV_or_LA(seg_1_before_processing, centroids[1])
    ...
    seg_2 = get_main_contour_MYO(seg_1, seg_2_before_processing) if num_of_contours1 > 0 else seg_2_before_processing   
    ...
    seg_total = combine_segmentations([seg_1, seg_2, seg_3], 'difference with overlap', [1, 2, 3])
...
def main_post_processing(path_to_segmentations, path_to_final_segmentations, single_frame_QC):
